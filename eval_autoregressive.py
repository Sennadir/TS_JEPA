...
if __name__ == "__main__":
    ...
    path_name = "/Auto_regressive_lr_" + str(config["lr_pretrain"]) \
                + "_encoder_" + str(config["pretrain_encoder_embed_dim"]) \
                + "_" + str(config["pretrain_encoder_nhead"]) \
                + "_" + str(config["pretrain_encoder_num_layers"]) \
    ...
    for epoch in range(num_epochs):
        ...
        for context_patches, target_patch in loader:
            ...
            loss = torch.nn.functional.mse_loss(predicted_next_patch, target_patch, reduction="mean")
    ...
    predictions = []
    current_context = loader.dataset.test_df[:context * num_patches].reshape(num_patches, context).unsqueeze(0)
    ...
    for step in range(num_steps):
        ...
        real_value = loader.dataset.test_df[context * num_patches + step * context: context * num_patches + (step + 1) * context]
        ...
        current_context = torch.cat([current_context[:, 1:], predicted_next_patch.unsqueeze(0)], dim=1)
